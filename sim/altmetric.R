sim<-function(mu=0,N=1000,ni=50,sl.sd=.3,gg=0,uu=1,xi.sd=0) {
    irf<-function(th,
                  a=1,
                  b,
...
mu<-runif(250,min=1,max=3)

library(parallel)
L1<-mclapply(mu,sim,mc.cores=10,N=5000)
...
pf2<-function(L,...) {
    ...
    ##
    plot(NULL,xlim=xl,ylim=c(-.005,.005),ylab='imv',xlab=expression(mu)); abline(h=0,lty=2,col='gray')
    pf(x$mu,x$om.r2,col='blue')
    pf(x$mu,x$om.23,col='red')
    legend("topright",bty='n',fill=c("blue","red"),c("(1PL,2PL)","(2PL,3PL)"))
    ##
    plot(NULL,xlim=xl,ylim=c(0,.02),ylab='rmsea',xlab=expression(mu)); abline(h=0,lty=2,col='gray')
