...
parfun<-function(args,simfun) {
    ...
    f<-function(y,pctt.tab) {
        ...
        for (ii in 0:(nn-1)) {
            ...
            p2<-y[[paste("graded",ii,sep='')]]
            p3<-y[[paste("gpcm",ii,sep='')]]
            true<-y[[paste("truep",ii,sep='')]]
            p0<-mean(resp0)
            ##
...
par(mfrow=c(2,1),mgp=c(2,1,0),mar=c(5,3,.1,.1),bty='n')
...
###########################
plot(NULL,xlim=c(1-3,nrow(z)),ylim=c(-.03,.03),xlab="",xaxt="n",ylab="IMV")
