...
pf2<-function(x,...,leg=TRUE) {
    xl<-range(x$mu)
    plot(NULL,xlim=xl,ylim=c(0,.12),ylab='Change in RMSE',...); abline(h=0,lty=2,col='gray')
    ...
    plot(NULL,xlim=xl,ylim=c(-.01,.03),ylab='Change in RMSEA',...); abline(h=0,lty=2,col='gray')
