parfun<-function(arg,numitems=50) {
    ...
    for (me in c("Rasch","2PL","3PL")) {
        ...
        names(y)[3]<-me
        f1<-M2(m)
        f2<-m@Fit[c("AIC",  "BIC", "SABIC", "DIC", "HQ","logLik")]
        fit[[me]]<-c(f2,f1)
        z[[me]]<-y                
    }
    x<-merge(z[[1]],z[[2]])
...
tab2<-list(parfun(argvals[[1]],numitems=50),
           parfun(argvals[[length(argvals)]],numitems=50)
           )
#library(parallel)
#mclapply(argvals,parfun,mc.cores=2,numitems=50)

for (i in 1:length(tab2)) {
    z<-tab2[[i]]
    tab<-z[[4]]
    ii<-which(rownames(tab)=="")
    rownames(tab)[ii]<-"IMV"
    nms<-c("logLik","AIC","BIC","SABIC","DIC","M2","RMSEA","TLI","CFI","IMV")
    index<-match(nms,rownames(tab))
    z[[4]]<-tab[index,]
    tab2[[i]]<-z
}
...
library(xtable)
xtable(tab2[[1]][[4]],digits=3)
xtable(tab2[[length(tab2)]][[4]],digits=3)

f<-function(tab) {
    apply(tab,1,which.min)
}
f(tab2[[1]][[4]])
f(tab2[[length(tab2)]][[4]])
